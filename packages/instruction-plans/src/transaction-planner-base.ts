...
import { appendTransactionMessageInstructions, type CompilableTransactionMessage } from '@solana/transaction-messages';
...
export type TransactionPlannerConfig = {
    ...
    }) => CompilableTransactionMessage | Promise<CompilableTransactionMessage>;
    newInstructionsTransformer?: <TTransactionMessage extends CompilableTransactionMessage>(
...
export function createBaseTransactionPlanner(config: TransactionPlannerConfig): TransactionPlanner {
    const createSingleTransactionPlan = async (
        ...
        const plan: SingleTransactionPlan = {
            kind: 'single',
            message: await Promise.resolve(config.createTransactionMessage({ abortSignal })),
        };
        if (instructions.length > 0) {
            abortSignal?.throwIfAborted();
            await addInstructionsToSingleTransactionPlan(plan, instructions, abortSignal);
        }
        return plan;
    };
    ...
    const addInstructionsToSingleTransactionPlan = async (
        ...
        let message = appendTransactionMessageInstructions(instructions, plan.message);
        if (config?.newInstructionsTransformer) {
            abortSignal?.throwIfAborted();
            message = await Promise.resolve(config.newInstructionsTransformer(plan.message, { abortSignal }));
        }
        (plan as Mutable<SingleTransactionPlan>).message = message as CompilableTransactionMessage;
    };

    return async (originalInstructionPlan, { abortSignal } = {}): Promise<TransactionPlan> => {
        ...
        if (!isValidTransactionPlan(plan)) {
            // TODO: Coded error.
            const error = new Error('Instruction plan results in invalid transaction plan') as Error & {
...
async function traverse(instructionPlan: InstructionPlan, context: TraverseContext): Promise<TransactionPlan | null> {
    ...
    switch (instructionPlan.kind) {
        ...
        default:
            ...
            throw new Error(`Unknown instruction plan kind: ${(instructionPlan as { kind: string }).kind}`);
    }
}
...
async function traverseSequential(
    ...
    if (mustEntirelyFitInCandidate) {
        ...
        candidate = allInstructions ? selectCandidate(context.parentCandidates, allInstructions) : null;
        if (candidate && allInstructions) {
            await context.addInstructionsToSingleTransactionPlan(candidate, allInstructions);
            return null;
        }
    } else {
...
async function traverseParallel(
    ...
    return parallelTransactionPlan(transactionPlans);
}
...
async function traverseIterable(
    ...
    while (iterator.hasNext()) {
        ...
        if (candidateResult) {
            ...
            await context.addInstructionsToSingleTransactionPlan(candidate, [ix], context.abortSignal);
        } else {
            const newPlan = await context.createSingleTransactionPlan([], context.abortSignal);
            const ix = iterator.next(newPlan.message);
            if (!ix) {
                throw new Error('Could not fit `InterableInstructionPlan` into a transaction');
            }
            await context.addInstructionsToSingleTransactionPlan(newPlan, [ix], context.abortSignal);
...
function getSequentialCandidate(latestPlan: TransactionPlan): SingleTransactionPlan | null {
...
function getParallelCandidates(latestPlan: TransactionPlan): SingleTransactionPlan[] {
    return getAllSingleTransactionPlans(latestPlan);
}

function getAllInstructions(instructionPlan: InstructionPlan): Instruction[] | null {
...
): [SingleTransactionPlan, Instruction] | null {
...
function selectCandidate(
    ...
    const firstValidCandidate = candidates.find(candidate => isValidCandidate(candidate, instructions));
    return firstValidCandidate ?? null;
}

function isValidCandidate(candidate: SingleTransactionPlan, instructions: Instruction[]): boolean {
...
export function getRemainingTransactionSize(message: CompilableTransactionMessage) {
...
function isValidTransactionPlan(transactionPlan: TransactionPlan): boolean {
    if (transactionPlan.kind === 'single') {
        const transactionSize = getTransactionMessageSize(transactionPlan.message);
