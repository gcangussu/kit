 ...
 *
 * It can accept {@link CompilableTransactionMessage} objects directly, which will be wrapped
 ...
 *
 * @example
 * Using {@link CompilableTransactionMessage | CompilableTransactionMessages} directly.
 * ```ts
 * const plan = parallelTransactionPlan([messageA, messageB]);
 * ```
 *
...
/**
 * Creates a {@link SingleTransactionPlan} from a {@link CompilableTransactionMessage} object.
 *
 * @example
 * ```ts
 * const plan = singleTransactionPlan(transactionMessage);
...
function parseSingleTransactionPlans(plans: (CompilableTransactionMessage | TransactionPlan)[]): TransactionPlan[] {
